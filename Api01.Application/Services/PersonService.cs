...
namespace Api01.Application.Services
{
    public class PersonService : IPersonService
    {
        ...
        {
            var result = await _personRepository.GetPeoplesAsync();
            return  ResultService.Ok<ICollection<PersonDTO>>(_mapper.Map<ICollection<PersonDTO>>(result));
        }
        ...
        public async Task<ResultService> UpadateAsync(PersonDTO personDTO)
        {
            if (personDTO == null)
            ...
            {
                return ResultService.RequestError("Problema com a validação dos camos!", validation);
            }
            ...
            person = _mapper.Map<PersonDTO, Person>(personDTO, person); //edicao
